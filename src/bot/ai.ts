...
=== Задача ===
Ты должен решить: стоит ли сейчас написать первым собеседнику? Если да — напиши сообщение.

Текущее время: ${formatTs(Date.now())}

НЕ пиши если:
- У человека ночь (22:00–08:00 по его времени, если неизвестно — по текущему времени: ${hour}:00)
- Разговор был недавно и нет повода писать
- Нет ничего уместного или интересного для собеседника

ПИШИ если:
- Есть что-то уместное и естественное сказать
- Прошло достаточно времени с последнего сообщения
- Это добавит ценность общению
...
Ответ ТОЛЬКО в JSON: {"write": true, "message": "текст сообщения"} или {"write": false}`;
  ...
  const msgs: { role: string; content: string }[] = [
    ...
    { role: "user", content: "Решение: писать или нет?" },
  ];
